...
public class CCTask extends Task {
   ...
   public void execute() throws BuildException {
	 ...
	 for (int i = 0; i < targetVectors.length; i++) {
	    ...
	    int requestedCores = getCores();
	    if (requestedCores > 0 && requestedCores < noOfCores)
	       noOfCores = requestedCores;

	    log("Using "+noOfCores+" processors");
	    if (maxCores > 0) {
   ...
   public int getCores() {
      return compilerDef.getCores(null, 0);
   }
